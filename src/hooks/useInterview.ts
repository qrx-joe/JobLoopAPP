...
export function useInterview() {
  ...
  const startSession = useCallback(
    async (jobTitle: string, resumeId?: string, jdId?: string): Promise<InterviewSession> => {
  ...
  const sendMessage = useCallback(
    async (sessionId: string, content: string, questionId?: string): Promise<InterviewMessage> => {
      store.setLoadingResponse(true);
